...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/19 15:50:33 by okuznets          #+#    #+#             */
/*   Updated: 2018/10/19 16:03:48 by okuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_pixel_web(unsigned char *pixel, int count, int max)
{
	double				intens;
	unsigned char		color;
...
void		*web_slave_body(t_param *param)
{
	int				x;
	int				y;
	double			x_real;
	...
	while (x < param->width)
	{
		...
		while (y < param->end_y)
		{
			...
			set_pixel_web(pixel, count_iterations_web(x_real, y_real,
...
void		*web_slave(void *param)
{
	return (web_slave_body(param));
}
...
void		web(t_image *image, t_window *window)
{
	...
	while (i < 4)
	{
		pthread_create(threads + i, NULL, &web_slave, param + i);
		i++;
	}
	i = 0;
	while (i < 4)
