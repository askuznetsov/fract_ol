...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/17 17:43:23 by okuznets          #+#    #+#             */
/*   Updated: 2018/10/19 14:38:32 by okuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_pixel_julia(unsigned char *pixel, int count, int max)
{
	double				intens;
	unsigned char		color;
...
void		*julia_slave_body(t_param *param)
{
	int				x;
	int				y;
	double			x_real;
	...
	while (x < param->width)
	{
		...
		while (y < param->end_y)
		{
			...
			set_pixel_julia(pixel, count_iterations_julia(param->window,
			x_real, y_real,
...
void		*julia_slave(void *param)
{
	return (julia_slave_body(param));
}
...
void		julia(t_image *image, t_window *window)
{
	...
	while (i < 4)
	{
		pthread_create(threads + i, NULL, &julia_slave, param + i);
		i++;
	}
	i = 0;
	while (i < 4)
