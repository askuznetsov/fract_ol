...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/19 15:30:05 by okuznets          #+#    #+#             */
/*   Updated: 2018/10/19 15:30:47 by okuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_pixel_fish(unsigned char *pixel, int count, int max)
{
	double				intens;
	unsigned char		color;
...
void		*fish_slave_body(t_param *param)
{
	int				x;
	int				y;
	double			x_real;
	...
	while (x < param->width)
	{
		...
		while (y < param->end_y)
		{
			...
			set_pixel_fish(pixel, count_iterations_fish(x_real, y_real,
...
void		*fish_slave(void *param)
{
	return (fish_slave_body(param));
}
...
void		fish(t_image *image, t_window *window)
{
	...
	while (i < 4)
	{
		pthread_create(threads + i, NULL, &fish_slave, param + i);
		i++;
	}
	i = 0;
	while (i < 4)
