...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/19 15:55:48 by okuznets          #+#    #+#             */
/*   Updated: 2018/10/19 16:02:42 by okuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_pixel_rose(unsigned char *pixel, int count, int max)
{
	double				intens;
	unsigned char		color;
...
void		*rose_slave_body(t_param *param)
{
	int				x;
	int				y;
	double			x_real;
	...
	while (x < param->width)
	{
		...
		while (y < param->end_y)
		{
			...
			set_pixel_rose(pixel, count_iterations_rose(x_real, y_real,
...
void		*rose_slave(void *param)
{
	return (rose_slave_body(param));
}
...
void		rose(t_image *image, t_window *window)
{
	...
	while (i < 4)
	{
		pthread_create(threads + i, NULL, &rose_slave, param + i);
		i++;
	}
	i = 0;
	while (i < 4)
