...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/19 15:37:52 by okuznets          #+#    #+#             */
/*   Updated: 2018/10/19 15:42:26 by okuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_pixel_anc(unsigned char *pixel, int count, int max)
{
	double				intens;
	unsigned char		color;
...
void		*anc_slave_body(t_param *param)
{
	int				x;
	int				y;
	double			x_real;
	...
	while (x < param->width)
	{
		...
		while (y < param->end_y)
		{
			...
			set_pixel_anc(pixel, count_iterations_anc(x_real, y_real,
...
void		*anc_slave(void *param)
{
	return (anc_slave_body(param));
}
...
void		anc(t_image *image, t_window *window)
{
	...
	while (i < 4)
	{
		pthread_create(threads + i, NULL, &anc_slave, param + i);
		i++;
	}
	i = 0;
	while (i < 4)
