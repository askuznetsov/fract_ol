...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/14 17:22:10 by okuznets          #+#    #+#             */
/*   Updated: 2018/10/19 14:29:07 by okuznets         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		set_pixel(unsigned char *pixel, int count, int max)
{
	double				intens;
	unsigned char		color;
...
void		*mandelbrot_slave_body(t_param *param)
{
	int				x;
	int				y;
	double			x_real;
	...
	while (x < param->width)
	{
		...
		while (y < param->end_y)
		{
			...
			set_pixel(pixel, count_iterations(x_real, y_real,
...
void		*mandelbrot_slave(void *param)
{
	return (mandelbrot_slave_body(param));
}
...
void		mandelbrot(t_image *image, t_window *window)
{
	...
	while (i < 4)
	{
		pthread_create(threads + i, NULL, &mandelbrot_slave, param + i);
		i++;
	}
	i = 0;
	while (i < 4)
